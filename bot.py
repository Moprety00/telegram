...
PROFESSIONS = ["Врач", "Учитель", "Программист", "Дизайнер", "Повар", "Полицейский", "Судья", "Адвокат", "Пожарный", "Пилот", "Космонавт", "Журналист", "Блогер", "Актёр", "Певец", "Архитектор", "Строитель", "Водитель", "Таксист", "Фермер", "Менеджер", "Модель", "Футболист", "Киберспортсмен", "Президент", "Хакер"]
...
def off_show_turn(uid, chat_id):
    ...
    if og['cur'] < og['players']:
        ...
        m = types.InlineKeyboardMarkup()
        m.add(types.InlineKeyboardButton("🔄 Заново", callback_data="off_restart"))
...
def start_distribution(lid):
    ...
    for p_id in p_ids:
        ...
        markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("👁️ Роль", callback_data="show_card"))
        msg = bot.send_message(p_id, "🎮 *Раунд начался!*", parse_mode="Markdown", reply_markup=markup)
